<x-app-layout>
    ...
    <form action="{{ route('plans.gifts.update', [$plan, $gift]) }}" method="POST" class="rounded pt-3 pb-8 mb-4"
        ...
        <div class="container lg:w-1/2 md:w-4/5 w-11/12 mx-auto mt-8 px-8 bg-blue-400 shadow-md rounded-md">
            <div class="mb-4">
                ...
                <input type="text" name="name"
                    ...
                    required placeholder="20字以上40字以下" value="{{ old('name', $gift->name) }}">
                <label class="block text-white mb-2" for="price">
                    金額
                </label>
                <input type="text" name="price"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-blue-800 w-full py-2 px-3"
                    required placeholder="リターンの金額を記入" value="{{ old('price', $gift->price) }}">
            </div>
            <div class="mb-4">
                ...
                <textarea name="description" rows="10"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-blue-800 w-full py-2 px-3"
                    required placeholder="？？文字以上??文字以下">{{ old('description', $gift->description) }}</textarea>
            </div>
            ...
            <div class="mb-4">
                <label class="block text-white mb-2" for="limited_befor">
                    募集方法
                </label>
